...
func createOrderItem(ctx context.Context, tx *sqlx.Tx, o OrderItem) error {
	res, err := tx.NamedExecContext(ctx, "INSERT INTO order_items (name, quantity, image, price, product_id, order_id) VALUES (:name, :quantity, :image, :price, :product_id, :order_id)", o)
	if err != nil {
		return fmt.Errorf("error insertin gorder item: %w", err)
	}
...
func (ms *MySQLStorer) execTx(ctx context.Context, fn func(*sqlx.Tx) error) error {
	...
	if err != nil {
		...
		if rbErr := tx.Rollback(); rbErr != nil {
			// if the rollback failed then show why rollback failed
			return fmt.Errorf("error rolling back transaction: %w", rbErr)
		}
		// else if the rollback didnt fail, show why transaction failed
		return fmt.Errorf("error in transaction: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("error committing transcation %w", err)
	}
